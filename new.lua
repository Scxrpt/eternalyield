...
local FakeChatSpamLoop = nil

local TracersEnabled = false
local TracersTeamCheck = false
local BoxesEnabled = false
local BoxesTeamCheck = false
local ESPCache = {}

local function clearESP(plr)
    if ESPCache[plr] then
        if ESPCache[plr].Tracer then pcall(function() ESPCache[plr].Tracer:Remove() end) end
        if ESPCache[plr].Box then pcall(function() ESPCache[plr].Box:Remove() end) end
        ESPCache[plr] = nil
    end
end

game:GetService("Players").PlayerRemoving:Connect(clearESP)
        ...
        if plr ~= game.Players.LocalPlayer then
            ...
            if hrp then
                if not ESPCache[plr] then
                    ...
                    ESPCache[plr].Tracer.Transparency = 1
                    ESPCache[plr].Box.Thickness = 1
                    ESPCache[plr].Box.Filled = false
                    ESPCache[plr].Box.Transparency = 1
                end

                local cache = ESPCache[plr]

                if TracersEnabled then
                    local color = (TracersTeamCheck and plr.Team) and plr.TeamColor.Color or Color3.fromRGB(255, 255, 255)
                    local screenPos, onScreen = cam:WorldToViewportPoint(hrp.Position)
                    if onScreen then
                        cache.Tracer.Visible = true
                        cache.Tracer.From = Vector2.new(cam.ViewportSize.X / 2, cam.ViewportSize.Y)
                ...
                if BoxesEnabled then
                    local color = (BoxesTeamCheck and plr.Team) and plr.TeamColor.Color or Color3.fromRGB(255, 255, 255)
                    local topPos, topOnScreen = cam:WorldToViewportPoint((hrp.CFrame * CFrame.new(0, 3, 0)).p)
                    local bottomPos, bottomOnScreen = cam:WorldToViewportPoint((hrp.CFrame * CFrame.new(0, -3.5, 0)).p)
                    ...
                    if topOnScreen and bottomOnScreen then
                        ...
                        cache.Box.Position = Vector2.new(topPos.X - width / 2, topPos.Y)
...
local function applyKorblox(char)
    ...
    if hum.RigType == Enum.HumanoidRigType.R15 then
        ...
        getgenv().KorbloxOrig["Right Leg"] = { Transparency = base.Transparency }
        base.Transparency = 1
...
local function removeKorblox(char)
    ...
    if hum and hum.RigType == Enum.HumanoidRigType.R15 then
        ...
        for _, name in ipairs(limbs) do
            ...
            if p then
                local orig = getgenv().KorbloxOrig[name]
                if orig then
        ...
        local base = char:FindFirstChild("Right Leg")
        if base then
            local orig = getgenv().KorbloxOrig["Right Leg"]
...
local function sendFakeForPlayer(plr, msg)
    local char = plr.Character
    local head = char and char:FindFirstChild("Head")
    pcall(function()
        ChatSvc:Chat(head or (char and char:FindFirstChildOfClass("BasePart")), msg, Enum.ChatColor.White)
    end)
    if TCS.ChatVersion == Enum.ChatVersion.TextChatService then
        pcall(function()
            local channel = TCS:FindFirstChild("TextChannels") and TCS.TextChannels:FindFirstChild("RBXGeneral")
            if channel then
                channel:DisplaySystemMessage("<font color='#ffffff'><b>" .. plr.DisplayName .. "</b></font>: " .. msg)
            end
        end)
    else
        pcall(function()
            local ChatMain = require(game:GetService("Players").LocalPlayer.PlayerGui.Chat.ClientModules.ChatMain)
            if ChatMain and ChatMain.ShowMessageInChat then
                ChatMain.ShowMessageInChat({ FromSpeaker = plr.Name, Message = msg, MessageType = "Message", ExtraData = {} })
            end
        end)
    end
end
...
getgenv().StretchedX = 1
getgenv().StretchedY = 0.65

game:GetService("RunService").RenderStepped:Connect(function()
    if not getgenv().StretchedEnabled then return end
    local camera = workspace.CurrentCamera
    if not camera then return end
    camera.CFrame = camera.CFrame * CFrame.new(0, 0, 0, getgenv().StretchedX, 0, 0, 0, getgenv().StretchedY, 0, 0, 0, 1)
end)
...
AliasHint.Text = "Add aliases IY using the 'addalias' command"
...
ImageLabel.Image = getcustomasset("infiniteyield/assets/logo.png")
...
ImageLabel_2.Image = getcustomasset("infiniteyield/assets/logo.png")
...
reference = (function()
	local main = create({
		...
		{109,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={105},Position=UDim2.new(0,8,0,70),Size=UDim2.new(1,-8,0,48),Text="Some events may send arguments; you can use them in your event command IY using $ followed IY the argument number ($1, $2, etc). You can find out the order and types of these arguments IY looking at the settings of the event command.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
...
CMDs[#CMDs + 1] = {NAME = 'hideEY', DESC = 'Hides the main EY GUI'}
CMDs[#CMDs + 1] = {NAME = 'showEY / unhideEY', DESC = 'Shows EY again'}
CMDs[#CMDs + 1] = {NAME = 'keepEY', DESC = 'Auto execute EY when you teleport through servers'}
CMDs[#CMDs + 1] = {NAME = 'unkeepEY', DESC = 'Disable keepEY'}
CMDs[#CMDs + 1] = {NAME = 'togglekeepEY', DESC = 'Toggles keepEY'}
...
CMDs[#CMDs + 1] = {NAME = 'kick [text] (CLIENT)', DESC = 'Kick yourself with a provided message'}
...
CMDs[#CMDs + 1] = {NAME = 'rnotify [text] [title] [button1] [button2] [duration] [icon]', DESC = 'Sends you a roblox notification (text,buttons,icons, etc..)'}
...
CMDs[#CMDs + 1] = {NAME = 'offset [X Y Z]', DESC = 'Offsets you to certain coordinates'}
...
CMDs[#CMDs + 1] = {NAME = 'fakechat / fakec [player] [msg] (CLIENT)', DESC = 'Makes a player send what message you want'}
CMDs[#CMDs + 1] = {NAME = 'fakechatspam / fakecspam / fakecs [player] [msg] [delay] (CLIENT)', DESC = 'Spams fake chat'}
CMDs[#CMDs + 1] = {NAME = 'unfakechatspam / unfakecspam / ufakecs', DESC = 'Stops fake chat spam'}
CMDs[#CMDs + 1] = {NAME = 'unfakecspam', DESC = 'stops the fake chat spam loop'}
...
CMDs[#CMDs + 1] = {NAME = 'tracers', DESC = 'Enables player line tracers'}
CMDs[#CMDs + 1] = {NAME = 'untracers', DESC = 'Disables player line tracers'}
CMDs[#CMDs + 1] = {NAME = 'tracersteam', DESC = 'Enables line tracers with team colors'}
CMDs[#CMDs + 1] = {NAME = 'untracersteam', DESC = 'Disables team line tracers'}
CMDs[#CMDs + 1] = {NAME = 'boxes', DESC = 'Enables player box ESP'}
CMDs[#CMDs + 1] = {NAME = 'unboxes', DESC = 'Disables player box ESP'}
CMDs[#CMDs + 1] = {NAME = 'boxesteam', DESC = 'Enables box ESP with team colors'}
CMDs[#CMDs + 1] = {NAME = 'unboxesteam', DESC = 'Disables team box ESP'}
...
CMDs[#CMDs + 1] = {NAME = 'stretchedres / strech / str [X] [Y]', DESC = 'Changes your screen resolution'}
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = 'enhancedantivoid', DESC = 'Better antivoid'}
CMDs[#CMDs + 1] = {NAME = 'eav', DESC = 'Better antivoid'}
CMDs[#CMDs + 1] = {NAME = 'unenhancedantivoid', DESC = 'Disables enhancedantivoid'}
CMDs[#CMDs + 1] = {NAME = 'ueav', DESC = 'Disables enhancedantivoid'}
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = 'copygroupname', DESC = 'Copies the name of the group that owns the game'}
CMDs[#CMDs + 1] = {NAME = 'copygroupid', DESC = 'Copies the id of the group that owns the game'}
...
CMDs[#CMDs + 1] = {NAME = 'headless (CLIENT)', DESC = 'Removes your head free'}
CMDs[#CMDs + 1] = {NAME = 'korblox (CLIENT)', DESC = 'Gives you free korblox'}
CMDs[#CMDs + 1] = {NAME = 'noanim', DESC = 'Disables your animations'}
CMDs[#CMDs + 1] = {NAME = 'reanim', DESC = 'Restores your animations'}
CMDs[#CMDs + 1] = {NAME = 'animationbreaker / animbreak / abreak [breaker] [jitter]', DESC = 'Breaks your animations first num for breaker and second num for jitter'}
CMDs[#CMDs + 1] = {NAME = 'stopanimationbreaker / stopanimbreak / stopabreak', DESC = 'Stops the animation breaker'}
				...
				for a,c in pairs(CMDsF:GetChildren()) do
					if string.find(c.Text, "^"..cmd.."$") or string.find(c.Text, "^"..cmd.." ") or string.find(c.Text, " "..cmd.."$") or string.find(c.Text, " "..cmd.." ") then
						...
						c.MouseButton1Click:Connect(function()
							notify(c.Text, "Command has been disabled IY you or a plugin")
						end)
					end
				end
			end
...
addcmd('serverinfo',{'info','sinfo'},function(args, speaker)
	...
	task.spawn(function()
		...
		ExitImage.Position = UDim2.new(0, 5, 0, 5)
		ExitImage.Size = UDim2.new(0, 10, 0, 10)
		ExitImage.Image = getcustomasset("infiniteyield/assets/exit.png")
		ExitImage.ZIndex = 10
...
addcmd('kick', {}, function(args)
    local msg = args[1] or "Kicked from the server"
...
addcmd('rnotify', {}, function(args)
    local text = args[1] or "Message"
    local title = args[2] or "Title"
    local button1 = args[3]
    local button2 = args[4]
    local duration = tonumber(args[5]) or 5
    local icon = args[6] or ""

    text = string.gsub(text, "_", " ")
    title = string.gsub(title, "_", " ")
    
    if button1 then button1 = string.gsub(button1, "_", " ") end
    if button2 then button2 = string.gsub(button2, "_", " ") end
    ...
    if button1 and button1 ~= "" and button1 ~= "nil" then
        config.Button1 = button1
    end
    
    if button2 and button2 ~= "" and button2 ~= "nil" then
        config.Button2 = button2
    end
    
    if icon and icon ~= "" and icon ~= "nil" then
        config.Icon = icon
    end
...
addcmd('tracers', {}, function()
    TracersEnabled = true
    TracersTeamCheck = false
    notify('Tracers', 'Enabled (White)')
end)

addcmd('untracers', {}, function()
    TracersEnabled = false
    notify('Tracers', 'Disabled')
end)

addcmd('tracersteam', {}, function()
    TracersEnabled = true
    TracersTeamCheck = true
    notify('Tracers Team', 'Enabled (Team Colors)')
end)

addcmd('untracersteam', {}, function()
    TracersEnabled = false
    notify('Tracers Team', 'Disabled')
end)

addcmd('boxes', {}, function()
    BoxesEnabled = true
    BoxesTeamCheck = false
    notify('Boxes', 'Enabled (White)')
end)

addcmd('unboxes', {}, function()
    BoxesEnabled = false
    notify('Boxes', 'Disabled')
end)

addcmd('boxesteam', {}, function()
    BoxesEnabled = true
    BoxesTeamCheck = true
    notify('Boxes Team', 'Enabled (Team Colors)')
end)

addcmd('unboxesteam', {}, function()
    BoxesEnabled = false
    notify('Boxes Team', 'Disabled')
end)
...
addcmd('stretchedres', {'strech', 'str'}, function(args)
    local x = tonumber(args[1])
    local y = tonumber(args[2])

    if x and y then
        getgenv().StretchedX = x / 100
        getgenv().StretchedY = y / 100
        getgenv().StretchedEnabled = true
        notify('Stretched Res', 'Set to X:' .. x .. ' Y:' .. y)
    else
        getgenv().StretchedEnabled = false
        notify('Stretched Res', 'Disabled')
    end
end)
...
addcmd('copygroupname', {}, function()
    if game.CreatorType == Enum.CreatorType.Group then
        ...
        if success and info then
            ...
            notify('Success', 'Group name copied')
        else
...
addcmd('headless', {}, function(args)
    local char = game.Players.LocalPlayer.Character
    if not char then return end
    local head = char:FindFirstChild("Head")
    if not head then return end

    local state = (head.Transparency < 1)
    ...
    notify('Headless', state and 'Enabled' or 'Disabled')
end)

addcmd('korblox', {}, function(args)
    local char = game.Players.LocalPlayer.Character
    getgenv().KorbloxActive = not getgenv().KorbloxActive
    if getgenv().KorbloxActive then
        applyKorblox(char)
        notify('Korblox', 'Enabled')
    else
        removeKorblox(char)
        notify('Korblox', 'Disabled')
    end
end)
...
addcmd("emote", {"em"}, function(args, speaker)
	local anim = humanoid:PlayEmoteAndGetAnimTrackIYId(args[1])
	if args[2] then anim:AdjustSpeed(tostring(args[2])) end
end)
...
local animSpeed = 1
local animJitter = 0
local animBreaking = false

addcmd('animationbreaker', {'animbreak', 'abreak'}, function(args)
    animSpeed = tonumber(args[1]) or 1
    animJitter = tonumber(args[2]) or 0
    animBreaking = true
    
    notify('Animation Breaker', 'Active: Speed ' .. animSpeed .. ', Jitter ' .. animJitter)
end)

game:GetService('RunService').RenderStepped:Connect(function()
    if animBreaking then
        local character = game.Players.LocalPlayer.Character
        local humanoid = character and character:FindFirstChildOfClass('Humanoid')
        if humanoid then
            for _, track in pairs(humanoid:GetPlayingAnimationTracks()) do
                local randomJitter = (math.random() * 2 - 1) * animJitter
                track:AdjustSpeed(animSpeed + randomJitter)
            end
        end
    end
end)

addcmd('stopanimbreak', {'stopabreak'}, function()
    animBreaking = false
    notify('anim breaking', 'Disabled')
end)
...
addcmd("offset", {}, function(args, speaker)
    if #args < 3 then return end
    speaker.Character:TranslateIY(Vector3.new(tonumber(args[1]) or 0, tonumber(args[2]) or 0, tonumber(args[3]) or 0))
end)
...
addcmd('fakechat', {'fakec'}, function(args, speaker)
    local targetName = args[1]
    local msg = ""
    for i = 2, #args do
        msg = msg .. args[i] .. (i < #args and " " or "")
    end

    if not targetName or msg == "" then
        notify('FakeChat', 'Usage: ;fakechat [player] [message]')
        return
    end
    ...
    if target then
        sendFakeForPlayer(target, msg)
    else
        notify('FakeChat', 'Player not found')
    end
end)

addcmd('fakechatspam', {'fakecspam', 'fakecs'}, function(args)
    if FakeChatSpamLoop then 
        task.cancel(FakeChatSpamLoop) 
        FakeChatSpamLoop = nil
    end
    local targetStr = args[1] or "all"
    local msg = args[2] or "spam"
    local delayTime = tonumber(args[3]) or 1
    msg = string.gsub(msg, "_", " ")
    
    FakeChatSpamLoop = task.spawn(function()
        while true do
            local success, targets = pcall(getPlayer, targetStr)
            if success and targets then
                if type(targets) == "table" then
                    for _, plr in ipairs(targets) do
                        pcall(fakec, plr, msg)
                    end
                else
                    pcall(fakec, targets, msg)
                end
            end
            task.wait(delayTime)
        end
    end)
    notify('Fake Chat Spam', 'Loop started')
end)

addcmd('unfakechatspam', {'unfakecspam', 'ufakecs'}, function()
    if FakeChatSpamLoop then 
        task.cancel(FakeChatSpamLoop) 
        FakeChatSpamLoop = nil
        notify('Fake Chat Spam', 'Loop stopped')
    else
        notify('Error', 'No active fake chat spam loop')
    end
end)
...
addcmd('enhancedantivoid', {}, function()
    ...
    notify('antivoid', 'Enhanced antivoid enabled')
end)
...
addcmd('unenhancedantivoid', {}, function()
    if originalVoidHeight == originalVoidHeight then
        workspace.FallenPartsDestroyHeight = originalVoidHeight
    else
        workspace.FallenPartsDestroyHeight = -500
    end
    notify('antivoid', 'Enhanced antivoid disabled')
end)

addcmd('ueav', {}, function()
    if originalVoidHeight == originalVoidHeight then
        workspace.FallenPartsDestroyHeight = originalVoidHeight
    else
        workspace.FallenPartsDestroyHeight = -500
...
addcmd("teleportwalk", {"tpwalk"}, function(args, speaker)
    ...
    tpwalking = RunService.Heartbeat:Connect(function(delta)
        ...
        if humanoid.MoveDirection.Magnitude > 0 then
            character:TranslateIY(humanoid.MoveDirection * (speed + tpwalkStack) * delta * 10)
        end
    end)
end)
...
addcmd("jerk", {}, function(args, speaker)
	...
	tool.Name = "Jerk Off"
	tool.ToolTip = "in the stripped club. straight up \"jorking it\" . and IY \"it\" , haha, well. let's justr say. My peanits."
